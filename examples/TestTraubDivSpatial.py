...
nml_doc, network = oc.generate_network("TestTraubDivSpatial")
...
popObjs=oc.add_populations_in_layers(network,boundaries,popDict,xs,zs,storeSoma=True)


extra_params=[{'pre':'L23PyrRS','post':'SupBasket','weights':[0.05],'delays':[5],'synComps':['NMDA'],
               'DistDependConn':'- 17.45 + 18.36 / (math.exp((r-267.)/39.) +1)'}]

conn_dict_Traub=[]
proj3={'type':'chem','PreCellGroup':'L23PyrRS','PostCellGroup':'SupBasket','SynapseList':['Syn_AMPA_SupPyr_SupFS','Syn_NMDA_RSPyr_SupFS'],'NumPerPreCell':8,'LocOnPostCell':'mid_tip_dends'}
conn_dict_Traub.append(proj3)

synapseList,projArray=oc.build_connectivity(network,popObjs,conn_dict_Traub,"../NeuroML2/prototypes/Thalamocortical/",extra_params)                  
